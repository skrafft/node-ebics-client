...
module.exports = class Client {
	...
	decipherAndUnzip(transactionKey, orderData) {
		const decipher = crypto.createDecipheriv('aes-128-cbc', transactionKey, DEFAULT_IV).setAutoPadding(false);
	...
	async download(order) {
		...
		let data = orderData.join('');

		return {
			orderData: this.decipherAndUnzip(transactionKey, data),
			orderId: res.orderId(),
